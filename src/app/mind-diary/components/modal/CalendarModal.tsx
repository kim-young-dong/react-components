...
import { getScorecalendar } from "@/apis/api/mind";
import { set } from "zod";
...
export default function CalendarModal({
  ...
  const [notCheckDays, setNotCheckDays] = useState<number[]>([]);
  useEffect(() => {
    fetchScoreCalendar();
  }, [date, isActive]);
  ...
  const fetchScoreCalendar = async () => {
    try {
      const res = await getScorecalendar(month + 1);

      setNotCheckDays(res.data.notCheckDays);
    } catch (error) {
      console.log(error);
    }
  };
    ...
    for (let i = 0; i < weeksInMonth; i++) {
      ...
      for (let j = 0; j < 7; j++) {
        if (i === 0 && j < startingDay) {
          ...
          const day = (
            <CalendarCell
              $notCheckDays={notCheckDays.includes(dayCounter)}
              $currentMonth={true}
              key={dayCounter}
              onClick={(e) => {
  ...
  return (
    <>
      <CalendarModalContainer $isActive={isActive}>
        <div className="w-[80%] min-w-[260px] z-10 p-5 bg-white rounded-xl">
